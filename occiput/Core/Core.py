...
from occiput.Visualization import MultipleVolumes, MultipleVolumesNiftyCore
...
from occiput.Core.NiftyCore_wrap import transform_grid, grid_from_box_and_affine, resample_image_on_grid
...
class GridND(object):
    ...
    def _repr_html_(self): 
        ...
        table_data = [[s,'','','','','',''],
                      ...
                      [str(self.space),str(self.shape),str(self.min()),str(self.max()),str(self.span()),str(self.center()),str(self.mean_dist_from_center())], ] 
        table = ipy_table.make_table(table_data)
        table = ipy_table.apply_theme('basic')
        table = ipy_table.set_cell_style(0,0, column_span=7)
        table = ipy_table.set_cell_style(0,0, align='center') 
...
class ImageND(object): 
    ...
    def get_pixel_grid(self): 
        ...
        grid = grid_from_box_and_affine(numpy.asarray([0,0,0]), self.shape, n_points) 
        return grid
...
class Image3D(ImageND):
    ...
    def compute_gradient_on_grid(self, grid, affine_grid_to_world=None, verify_mapping=True): 
        ...
        gradient_data = numpy.gradient(resampled_data) #FIXME: use NiftyCore
    ...
    def compute_gradient_pixel_space(self):
        gradient_data = numpy.gradient(self.data)  #FIXME: use NiftyCore
        return gradient_data
